...
void Init(std::string solver_state) {
  ...
  solver = boost::shared_ptr<caffe::Solver<float>>(caffe::SolverRegistry<float>::CreateSolver(param));
...
void AddReplay(std::pair<int, int> cp, int action_index, std::pair<int, int> np, float reward) {
  ...
  boost::shared_ptr<Replay> replay = boost::shared_ptr<Replay>(new Replay(cp, action_index, np, reward));
...
void Update() {
  ...
  for(int i = 0; i < mini_batch_size; ++i) {
    if(reps[i]->nextPosition.first == reps[i]->nextPosition.second &&
        (reps[i]->nextPosition.first == 0 || reps[i]->nextPosition.first == 3)) {
      continue;
    }
  ...
  for(int i = 0; i < mini_batch_size; i++) {
    ...
    cout << "input : " << reps[i]->currentPosition.first << ", " << reps[i]->currentPosition.second << endl;
    cout << "labelQvalue : ";
    for(int j = 0; j < 4; ++j) {
      cout << label[i * 4 + j] << " \n"[j == 3];
    }
